export const BASE_URL = "http://localhost:3000/";

// END POINTS

export const SIGNUP = "/api/auth/signup";
export const LOGIN = "/api/auth/login";

export const ADMIN = "/api/v1/admin";
export const ADMINPRODUCTSAPI = "/api/v1/admin/products/"

export const FORGOTPASSWORD = "/api/auth/forgot-password";

export const CARTAPI = "/api/v1/cart/";
export const PRODUCTSAPI = "/api/v1/products/";

export const CARTITEMSAPI = "/api/v1/cart/items/";
export const CHECKOUTAPI = "/api/v1/checkout";